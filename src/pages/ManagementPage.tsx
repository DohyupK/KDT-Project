...
// # 타입 정의 (데이터가 어떤 모양인지 알려줌)
type MailImportance = '높음' | '보통' | '낮음';
type InquiryStatus = '대기' | '진행중' | '완료';
type InquiryPriority = '높음' | '보통' | '낮음';
type TabId = 'mail' | 'inquiry' | 'defect'; // # 상단 탭 종류
type StatusFilter = '전체' | InquiryStatus; // # 문의 필터 종류
...
// # 지금 시각을 문자열로 만들기 (예: 2026-07-16 15:30)
function formatNow() {
  const d = new Date();
...
// # 알림 규칙: 같은 라인이 최근 3일 연속으로 임계값을 넘으면 알림 대상
...
export const ManagementPage = () => {
  ...
  // # 화면 상태들 (클릭/입력하면 바뀌는 값)
  const [activeTab, setActiveTab] = useState<TabId>('mail'); // # 지금 보고 있는 탭
  const [mails, setMails] = useState<MailItem[]>(INITIAL_MAILS); // # 메일 목록
  const [selectedMailId, setSelectedMailId] = useState<string | null>(null); // # 선택한 메일
  const [inquiries, setInquiries] = useState<InquiryItem[]>(INITIAL_INQUIRIES); // # 문의 목록
  const [statusFilter, setStatusFilter] = useState<StatusFilter>('전체'); // # 문의 상태 필터
  const [selectedInquiryId, setSelectedInquiryId] = useState<string | null>(null); // # 선택한 문의
  const [replyForm, setReplyForm] = useState<ReplyFormState>(EMPTY_REPLY_FORM); // # 답변 입력값
  const [threshold, setThreshold] = useState(3); // # 불량률 임계값 (%)
  const [n8nEnabled, setN8nEnabled] = useState(true); // # n8n 모니터링 ON/OFF
                    ...
                    {!selectedMail ? (
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                          <div>
                            <p className="text-gray-400 text-xs">메일ID</p>
                            <p className="font-medium">{selectedMail.id}</p>
                          </div>
                          <div>
                            <p className="text-gray-400 text-xs">수신일시</p>
                            <p className="font-medium">{selectedMail.receivedAt}</p>
                          </div>
                          <div>
                            <p className="text-gray-400 text-xs">발신자</p>
                            <p className="font-medium">{selectedMail.sender}</p>
                          </div>
                          <div>
                            <p className="text-gray-400 text-xs">참조(CC)</p>
                          ...
                          filteredInquiries.map((item) => (
                            ...
                            >
                              ...
                              <td className="px-4 py-2.5">
                                <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${badgeStatus(item.status)}`}>
                                  {item.status ?? '대기'}
                                </span>
                              </td>
                              <td className="px-4 py-2.5">
                                <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${badgePriority(item.priority)}`}>
                                  {item.priority ?? '보통'}
                                </span>
                              </td>
                    ...
                    {!selectedInquiry ? (
                      ...
                      <div className="space-y-3 text-sm">
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <p className="text-xs text-gray-400">문의ID</p>
                          ...
                          <div>
                            <p className="text-xs text-gray-400">처리 부서</p>
                            <p className="font-medium">{selectedInquiry.department ?? '-'}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-400">작성일 / 수정일</p>
