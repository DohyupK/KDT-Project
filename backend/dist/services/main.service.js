...
exports.getMainOverview = getMainOverview;
...
function buildAiInsight(lot, defectRate) {
...
function buildAlerts(defectLots, defectRate, predictedRate, latestLotId) {
    const alerts = [];
    ...
    defectLots.slice(0, 3).forEach((row, index) => {
        if (alerts.some((a) => a.lotId === row.lot_id))
            return;
        alerts.push({
...
async function fetchOverviewFromDb() {
    const latestRows = await (0, connection_1.query)(`SELECT lot_id, post_sintering_at, sintering_temp, lithium_input, quality_defect
     FROM cathode_classification_data
     ORDER BY post_sintering_at DESC
     LIMIT 1`);
    const defectLotRows = await (0, connection_1.query)(`SELECT lot_id, post_sintering_at, sintering_temp, lithium_input, quality_defect
     FROM cathode_classification_data
     WHERE quality_defect = 1
     ORDER BY post_sintering_at DESC
     LIMIT 5`);
    const actualRateRows = await (0, connection_1.query)(`SELECT daily_defect_rate, record_date, rate_type
     FROM daily_defect_rates
     WHERE rate_type = 'ACTUAL'
     ORDER BY record_date DESC
     LIMIT 1`);
    const predictedRateRows = await (0, connection_1.query)(`SELECT daily_defect_rate, record_date, rate_type
     FROM daily_defect_rates
...
async function getMainOverview() {
    try {
        ...
        if ((0, db_1.useMockStorage)('MOCK_MAIN') || (0, db_1.isDbUnavailableError)(err)) {
            return MOCK_OVERVIEW;
        }
        throw err;
    }
