...
- **테이블:** `lots` PK=`id` (공정만) · 채점=`analysis_lots` · 잔류=`judgment_lots` · 자식 FK `lot_id` → `lots.id`
- **이슈 ID:** `ISS-yyMMdd-001` 일별 순번 유지
- 채점 입력: `lots` 공정값 → `/predict`·`/predict-capacity`·`/predict-residual` → `analysis_lots` UPSERT + `judgment_lots` **NULL만** 채움
- LOT CSV 적재: `POST /api/lots/import` (`id`/`timestamp`/공정 → `lots`)
- QC 재적재: `npm run reload:lots-qc` · `../DB/reload_lots_from_qc_csv.sql`
- SPC 싱크: backend 기동 시 **60초 폴링** (`SPC_SYNC_ENABLED` 기본 on · `SPC_SYNC_INTERVAL_MS=60000`) · 수동 `npm run sync:spc-lots`
