...
function buildInitialDocuments(): KnowledgeDocument[] {
  return [
    {
      id: 'DOC-2026-041',
      manager: '김현수',
...
function buildInitialActions(): KnowledgeAction[] {
  return [
    {
      id: 1,
...
const INITIAL_REPORT: KnowledgeReport = {
  baseDate: '2026-07-21',
  mainCause:
    '소성 온도 상한 근접 운전(747°C 이상) 구간에서의 결정성 저하가 최근 불량률 상승분의 62%를 설명합니다.',
  similarCase: '2026-07-18 소성로 2호기 온도 초과 사례(DOC-2026-041)와 공정 패턴이 91% 유사합니다.',
  recommendation:
    '3구간 목표 온도를 742°C로 유지하고, 승온 속도를 5% 하향한 상태에서 4시간 간격으로 결정성 샘플링을 권장합니다.',
  riskSummary: '현재 위험도 중간 — 조치 미이행 시 48시간 내 불량률 2.5% 초과 확률 78%',
  referenceCount: 126,
}

const memoryDocuments: KnowledgeDocument[] = buildInitialDocuments()
const memoryActions: KnowledgeAction[] = []
let memoryReport: KnowledgeReport = { ...INITIAL_REPORT }
...
function mapActionRow(row: ActionRow): KnowledgeAction {
  return {
    id: row.id,
...
function filterDocuments(
  ...
  params: { manager?: string; date?: string; keyword?: string },
) {
...
async function loadActionsFromDb(): Promise<KnowledgeAction[]> {
  const rows = await query<ActionRow[]>(
    'SELECT * FROM knowledge_actions ORDER BY action_date DESC, id DESC',
  )
  return rows.map(mapActionRow)
}

async function seedActionsToDb(actions: KnowledgeAction[]) {
  for (const item of actions) {
    await query(
      `INSERT INTO knowledge_actions (situation, action, cause, manager, action_date)
       VALUES (?, ?, ?, ?, ?)`,
      [item.situation, item.action, item.cause, item.manager, item.date],
    )
  }
}
...
async function ensureMemoryLoaded() {
  ...
  try {
    let dbActions = await loadActionsFromDb()
    if (dbActions.length === 0 && !useMockStorage('MOCK_KNOWLEDGE')) {
      await seedActionsToDb(buildInitialActions())
      dbActions = await loadActionsFromDb()
    }
    if (dbActions.length > 0) {
      memoryActions.splice(0, memoryActions.length, ...dbActions)
      actionIdCounter = Math.max(0, ...dbActions.map((a) => a.id))
    }
  } catch (err) {
    if (!(useMockStorage('MOCK_KNOWLEDGE') || isDbUnavailableError(err))) throw err
  }

  if (memoryActions.length === 0) {
    memoryActions.push(...buildInitialActions())
    actionIdCounter = Math.max(actionIdCounter, ...memoryActions.map((a) => a.id))
  }
}

function getManagers(documents: KnowledgeDocument[]) {
...
export async function getDocuments(params: {
  ...
  try {
    ...
    if (useMockStorage('MOCK_KNOWLEDGE') || isDbUnavailableError(err)) {
...
export async function getDocumentById(id: string) {
  ...
  if (!doc) throw new AppError(404, '문서를 찾을 수 없습니다.')
  return doc
}
...
export async function createAction(input: {
  ...
  if (!input.situation.trim()) throw new AppError(400, '발생 상황을 입력해주세요.')
  if (!input.action.trim()) throw new AppError(400, '대처 방안을 입력해주세요.')
  if (!input.cause.trim()) throw new AppError(400, '원인을 입력해주세요.')
  if (!input.manager.trim()) throw new AppError(400, '담당자를 입력해주세요.')
  if (!input.date) throw new AppError(400, '날짜를 입력해주세요.')

  await ensureMemoryLoaded()
  actionIdCounter += 1
  const created: KnowledgeAction = {
    id: actionIdCounter,
  ...
  try {
    const result = (await query(
      `INSERT INTO knowledge_actions (situation, action, cause, manager, action_date)
       VALUES (?, ?, ?, ?, ?)`,
      [created.situation, created.action, created.cause, created.manager, created.date],
    )) as { insertId?: number | bigint }
    if (result.insertId) created.id = Number(result.insertId)
...
export async function updateAction(
  id: number,
  input: { situation: string; action: string; cause: string; manager: string; date: string },
) {
  if (!input.situation.trim()) throw new AppError(400, '발생 상황을 입력해주세요.')
  if (!input.action.trim()) throw new AppError(400, '대처 방안을 입력해주세요.')
  if (!input.cause.trim()) throw new AppError(400, '원인을 입력해주세요.')
  ...
  if (index < 0) throw new AppError(404, '상황 대처 이력을 찾을 수 없습니다.')

  const updated: KnowledgeAction = {
    id,
    situation: input.situation.trim(),
    action: input.action.trim(),
    cause: input.cause.trim(),
  ...
  try {
    await query(
      `UPDATE knowledge_actions SET situation = ?, action = ?, cause = ?, manager = ?, action_date = ? WHERE id = ?`,
      [updated.situation, updated.action, updated.cause, updated.manager, updated.date, id],
    )
...
export async function deleteAction(id: number) {
  ...
  try {
    await query('DELETE FROM knowledge_actions WHERE id = ?', [id])
