...
interface IssueRow {
  ...
  completed: number
  anomaly: string
  process_data: string | ProcessDataPayload[]
}

const ISSUE_STATUSES = ['접수', '분석 중', '조치 중', '완료'] as const
const ISSUE_RISKS = ['높음', '중간', '낮음'] as const
...
const MOCK_HANDOVER: HandoverSummary = {
...
function buildInitialIssues(): IssuePayload[] {
  return [
    {
      id: 'ISS-260721-018',
      ...
      completed: false,
      anomaly:
        '14시 이후 온도가 관리 상한 750°C를 3회 초과했으며 AI 위험 점수가 91점까지 상승했습니다.',
    ...
    },
    {
      ...
      completed: false,
      anomaly:
        '리튬 투입 속도의 표준편차가 기준 대비 32% 증가하여 조성 불균일 가능성이 감지되었습니다.',
      processData: createProcessData(
    ...
    },
    {
      ...
      completed: false,
      anomaly:
        '냉각수 압력이 2.7bar까지 급상승하고 배출 온도 안정화 시간이 평소보다 18분 지연되었습니다.',
      processData: createProcessData(
...
const memoryIssues: IssuePayload[] = []

function parseProcessData(value: string | ProcessDataPayload[] | null): ProcessDataPayload[] {
  if (!value) return []
  if (Array.isArray(value)) return value
  try {
    const parsed = JSON.parse(value) as unknown
    return Array.isArray(parsed) ? (parsed as ProcessDataPayload[]) : []
  } catch {
    return []
  }
}
...
function mapRow(row: IssueRow): IssuePayload {
  ...
  return {
    ...
    lot: row.lot,
    risk: row.risk as IssuePayload['risk'],
    status: row.status as IssuePayload['status'],
    title: row.title,
...
function upsertMemoryIssue(issue: IssuePayload) {
  const index = memoryIssues.findIndex((item) => item.id === issue.id)
  if (index >= 0) memoryIssues[index] = issue
  else memoryIssues.unshift(issue)
}
...
function validateStatus(status: string) {
  if (!ISSUE_STATUSES.includes(status as (typeof ISSUE_STATUSES)[number])) {
    throw new AppError(400, '유효하지 않은 처리 상태입니다.')
  }
}

async function fetchAllFromDb(): Promise<IssuePayload[]> {
  const rows = await query<IssueRow[]>('SELECT * FROM issues ORDER BY occurred_at DESC')
  return rows.map(mapRow)
}

async function seedIssuesToDb(issues: IssuePayload[]) {
  for (const issue of issues) {
    await query(
      `INSERT INTO issues (id, occurred_at, lot, risk, status, title, assignee, action, completed, anomaly, process_data)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        issue.id,
        `${issue.occurredAt}:00`,
        ...
        issue.completed ? 1 : 0,
        issue.anomaly,
        JSON.stringify(issue.processData),
      ],
    )
  }
}
...
async function seedMemoryFromDbOrMock() {
  try {
    ...
    if (!useMockStorage('MOCK_ISSUES')) {
      await seedIssuesToDb(buildInitialIssues())
      const seeded = await fetchAllFromDb()
      if (seeded.length > 0) {
        memoryIssues.splice(0, memoryIssues.length, ...seeded)
        return
      }
    }
  } catch (err) {
    if (!(useMockStorage('MOCK_ISSUES') || isDbUnavailableError(err))) throw err
  }
...
async function getAllIssuesInternal(): Promise<IssuePayload[]> {
...
export async function getIssues(params: {
  ...
  try {
    ...
    if (useMockStorage('MOCK_ISSUES') || isDbUnavailableError(err)) {
...
export async function getIssueById(id: string) {
  ...
  if (!issue) throw new AppError(404, '이슈를 찾을 수 없습니다.')
  return issue
}
...
export async function updateIssue(
  ...
  const status = input.completed ? '완료' : (input.status as IssuePayload['status'])
  const updated: IssuePayload = {
    ...existing,
  ...
  try {
    await query(
      `UPDATE issues SET assignee = ?, status = ?, action = ?, completed = ? WHERE id = ?`,
      [updated.assignee, updated.status, updated.action, updated.completed ? 1 : 0, id],
    )
...
export async function getHandoverSummary(): Promise<HandoverSummary> {
  try {
    ...
    if (count === 0) return MOCK_HANDOVER
