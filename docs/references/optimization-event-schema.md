# optimization_events 계약

최종 갱신: 2026-08-14  
적용: `backend` 제어 승인 로그 (`CONTROL_STORE` 또는 `CHAT_STORE` → sqlite 기본)

Approve(제안 승인) 시 하드웨어로 보내지 않는다. 로그만 남긴다.  
...
| `prob_before` / `prob_after` | O/X 불량 확률 |
| `capacity_before` / `capacity_after` | reg **예측** 용량 |
| `residual_before` / `residual_after` | residual **예측** ppm |
| `method` | `whatif_grid` |
| `status` | `approved` \| `reverted` |
| `outcome_quality_defect` | **실측** 0/1 |
| `outcome_capacity` | **실측** mAh/g (선택) |
| `outcome_residual_li` | **실측** ppm (선택) |
| `created_at` | 시각 |
