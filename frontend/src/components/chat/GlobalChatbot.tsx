...
const USAGE_GUIDE_TEXT =
  ...
  '3. LOT ID를 입력하면 공정값·위험도·잔류 리튬을 조회하고, LOT ID 두 개를 입력하면 차이를 비교합니다.\n' +
  '4. 이슈 ID·문의 ID로 상세를 조회하거나 현재 메뉴의 미처리 현황과 우선순위를 물어볼 수 있습니다.\n' +
  '5. 답변은 조회 결과·판단 근거·권장 확인 순서로 제공되며 데이터 변경은 수행하지 않습니다.\n' +
  '6. 공개·대외비 문서는 RAG로 검색됩니다. 기밀은 「보안 상담」을 이용하세요.\n' +
  '7. 추가 질문으로 모델 진단·What-if를 요청할 수 있습니다.'

const WELCOME_GENERAL: ChatMessage = {
  id: 1,
  role: 'ai',
  text:
    '안녕하세요, YAHO입니다.\n\n화면 요약뿐 아니라 LOT 비교, 이슈·문의 조회, 우선 점검 항목과 근거도 확인할 수 있습니다.',
}

type ChatMode = 'general' | 'secure'

const LOCAL_THREADS_KEY = 'kdt_general_chat_recent_threads'
const LEGACY_DELETED_THREADS_KEY = 'kdt_general_chat_deleted_threads'
...
export default function GlobalChatbot() {
  ...
  const refreshThreads = async () => {
    if (typeof window !== 'undefined') {
      window.localStorage.removeItem(LEGACY_DELETED_THREADS_KEY)
    }
  ...
  const deleteThread = async (threadId: string) => {
    if (pending) return
    try {
      await deleteChatThread(threadId, 'general')
      ...
      console.warn('[general-chat] server delete failed', err)
      void refreshThreads()
    }
  }
  ...
  if (pathname.includes('/main') || pathname.includes('/dashboard')) {
    chips.push({
      label: '위험 LOT 분석',
      message: '현재 화면의 위험 LOT 중 우선 확인할 항목과 판단 근거, 권장 점검을 알려주세요.',
      features: null,
    })
  } else if (pathname.includes('/issue')) {
    chips.push({
      label: '열린 이슈 우선순위',
      message: '현재 화면 필터의 열린 이슈 현황과 우선 처리 순서, 다음 조치를 알려주세요.',
      features: null,
    })
  } else if (pathname.includes('/inquiry')) {
    chips.push({
      label: '문의 처리 현황',
      message: '현재 문의 게시판의 상태별 현황과 우선 답변할 문의를 알려주세요.',
      features: null,
    })
  } else if (pathname.includes('/knowledge')) {
    chips.push({
      label: '근거 문서 분석',
      message: '현재 선택된 자료와 관련 문서의 핵심 근거, 실무 확인 사항을 분석해 주세요.',
      features: null,
    })
  } else if (pathname.includes('/management')) {
    chips.push({
      label: 'SPC 점검',
      message: '현재 SPC 화면에서 우선 확인할 패널과 점검 순서를 알려주세요.',
      features: null,
    })
  }
