"""
Capacity regression: train_model() + predict_capacity(df)

CWD: ai-service/
Data: data/cathode_reg_data.csv
Artifacts: models/reg/
...
MODEL_VERSION = "1.0.0-reg"
OPTUNA_TRIALS = int(os.environ.get("OPTUNA_TRIALS", "100"))
DATA_PATH = Path("data/cathode_reg_data.csv")
MODELS_DIR = Path("models/reg")
LOGS_DIR = Path("logs")
OPTUNA_DB = "sqlite:///optuna_reg.db"
TARGET_COL = "capacity"
...
def setup_logging() -> None:
    ...
    log_path = LOGS_DIR / "train_reg.log"
...
def tune_xgboost(X: np.ndarray, y: np.ndarray, device_mode: str) -> dict[str, Any]:
    ...
    if remaining > 0:
        study.optimize(
            ...
            callbacks=[_optuna_progress_callback("xgb_cap_reg", target)],
        )
...
def tune_catboost(
    ...
    if remaining > 0:
        study.optimize(
            ...
            n_trials=remaining,
            show_progress_bar=False,
            callbacks=[_optuna_progress_callback("cat_cap_reg", target)],
        )
    return dict(study.best_params)
...
def _load_artifacts_if_needed() -> None:
    ...
    if missing:
        raise FileNotFoundError(
            "Reg model artifacts missing. Run train_model() first. Missing: "
            + ", ".join(missing)
        )
...
def predict_capacity(df: pl.DataFrame) -> dict[str, Any]:
    """Realtime capacity (mAh/g) inference. Single-row polars DataFrame only."""
    if not isinstance(df, pl.DataFrame):
        raise TypeError("df must be a polars.DataFrame")
    if df.height != 1:
        raise ValueError(f"predict_capacity expects exactly 1 row; got {df.height}")
    ...
    capacity = float(0.5 * p_xgb + 0.5 * p_cat)
